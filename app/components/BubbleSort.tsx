...
export default function BubbleSort({ initialArray, prettyCode }: { initialArray: number[], prettyCode: string[] }) {
  ...
  const formattedInitialArray = [{ entries: initialArray.map(elem => ({ value: elem, color: unsortedStateColor })), lineNo: [], explanation: '' }];
  ...
  async function resetArray() {
    ...
    const updatedArray = [{ entries: generateRandomUniqueArray({ length: bubbleSortArrayLength }).map(elem => ({ value: elem, color: unsortedStateColor })), lineNo: [], explanation: '' }];
    setStateArray([]);
    ...
    <section>
      ...
      <div className="flex">
        ...
        <div className="p-9 flex-1">
          <p className="text-lg">{exampleArray[0].explanation}</p>
        </div>
      </div>
