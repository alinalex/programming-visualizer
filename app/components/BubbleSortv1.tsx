...
export default function BubbleSort({ initialArray }: { initialArray: number[] }) {
  ...
  const len = useRef<number>(staticArray.length - 1);
  ...
  async function reorder() {
    ...
    do {
      swapped = false;
      for (let i = currentIndex.current || 0; i < len.current; i++) {
        setIsCompared([i, i + 1]);
        await timer(bubbleSortWaitMs);
        if (staticArray[i] > staticArray[i + 1]) {
          let toBeUpped = staticArray[i];
          let toBeDown = staticArray[i + 1];
          const updatedArray = [...staticArray.slice(0, i), toBeDown, toBeUpped, ...staticArray.slice(i + 2)];
          staticArray = updatedArray;
          setExampleArray(updatedArray)
          await timer(bubbleSortWaitMs);
          swapped = true;
        }
        if (currentIndex.current !== null) currentIndex.current = null;
        if (pause.current) {
          currentIndex.current = i + 1;
          break;
        }
        // if (i === staticArray.length - 2) {
        //   setIsCompared([]);
        //   setSortedIndexes(prevState => [...prevState, staticArray.length - i]);
        //   await timer(bubbleSortWaitMs);
        // }
